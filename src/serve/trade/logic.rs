...
impl trade_logic for Trade {
    ...
    fn remove_from_trade(& mut self, owner : &OwnerModel, item :  &PossessionModel) -> bool{
        if self.trader_1.id == owner.id {
            if let Some(i) = self.trade_1_items.iter().position(|value| *value == item.id) {
            ...
            if let Some(i) = self.trade_2_items.iter().position(|value| *value == item.id) {
